...
private val TRACK_COLOR = Color.White.copy(alpha = .7f)
...
fun <T : Any> SegmentedControl(
    ...
    state.onSegmentSelected = {
        haptic.performHapticFeedback(HapticFeedbackType.LongPress)
        onSegmentSelected(segments[it])
    }
...
private suspend fun AwaitPointerEventScope.waitForUpOrCancellation(inBounds: Rect): PointerInputChange? {
    while (true) {
        ...
        if (event.changes.any { it.changedToDown() || !inBounds.contains(it.position) }) {
